...
def train(model):
    ...
    # 學習參數
    epsilon = 0.9  # 探索率，隨機選擇動作的機會
    epoch_start = 0  # 訓練的起始輪次
    n_epoch = 10_000  # 總訓練輪數
    epsilon_decay = 0.999  # 探索率衰減係數
    min_epsilon = 0.1  # 最小探索率
    discount = 0.9  # 折扣因子，用於計算未來回報
    ...
    n_win_games = 0  # 記錄贏得的比賽數量
    game_result_list = []  # 記錄每場比賽的結果
    game_length_list = []  # 記錄每場比賽的持續回合數

    n_epoch_update_model = 20  # 更新目標模型的週期
    n_epoch_save_model = 300  # 保存模型的週期
    ...
    for epoch in range(epoch_start, n_epoch):
        ...
        win_ratio = n_win_games / (epoch + 1) * 100  # 計算勝率

        game_length_list.append(len(rewards))  # 記錄遊戲長度
